...
# --- APP TITLE ---
st.title("🎄 Winter Vinyl Jukebox: Tune Your Holiday Vibe")
st.subheader("❄️ Your personal station for Lofi, City Pop, and Ambient Jazz.")

# --- UI Controls ---
col1, col2 = st.columns(2)

with col1:
    st.markdown("### 1. Capture the Moment's Energy 📸")
    st.markdown("---")
    ...
    mood_vibe = st.selectbox(
        ...
        options=[
            "✨ Excited & Sparkling (High Energy)",
            "☕ Calm & Pondering (Medium Focus)",
            "☁️ Cozy & Lazy (Low Chill)"
        ],
        index=1,
    ...
    st.info(f"**Your Vibe:** {mood_vibe.split(' ')[0].replace('✨', '').replace('☕', '').replace('☁️', '').strip()}")


with col2:
    st.markdown("### 2. Where is Your Turntable Spinning? 📍")
    st.markdown("---")
    ...
    location = st.radio(
        ...
        options=(
            "🏠 By the Cozy Fireplace (Home)",
            "🚌 Drifting Through the City (Commuting)",
            "🏢 Going to Work (Work)", # NEW OPTION
            "🌲 Snowy Park / Outdoors",
            "📚 Quiet Study Corner (Cafe/Study)"
        ),
    ...
    st.info(f"**Your Scene:** {location.split(' ')[1].replace('🏠', '').replace('🚌', '').replace('🏢', '').replace('🌲', '').replace('📚', '').strip()}")

st.markdown("---")

# --- Music Recommendation Button ---
if st.button("🎁 Open Your Personalized Jazz Box"):
